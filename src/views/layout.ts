...
type NavKey = 'explore' | 'my-items' | 'history';
...
export class Layout {
  ...
  constructor(router: Navigo) {
    ...
    // 데스크톱 내비 (가운데)
    const makeBtn = (label: string, key: NavKey) =>
      el('sl-button', label, { variant: 'text', size: 'large', className: 'nav-btn', 'data-key': key });

    const nav = el('nav.primary-nav',
      makeBtn('탐색', 'explore'),
      makeBtn('내 NFT', 'my-items'),
      makeBtn('히스토리', 'history'),
    );
    ...
    this.drawer.addEventListener('sl-select', (e: any) => {
      const key = e.detail?.item?.dataset?.key as NavKey | undefined;
      if (key) {
        router.navigate(`/${key}`);     // ← 이동 추가
        (this.drawer as any).hide();    // 드로어 닫기
      }
    });
    ...
    nav.querySelectorAll('.nav-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const key = (e.currentTarget as HTMLElement).dataset.key as NavKey | undefined;
        if (key) router.navigate(`/${key}`);
      });
    });
